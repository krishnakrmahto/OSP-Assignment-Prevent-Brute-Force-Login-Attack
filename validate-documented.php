	...
	class login_attempt
	{
		...
		private function addToQueue()
		{
			...
			else
			{
				...
				$query_result = mysqli_query($connection, $query) or die(mysqli_error($connection)); // dies when ip address already exists, so from the same ip address, the hacker won't be able to make multiple attempts to login
		...
		private function isReady()
		{
			...
			{
				$query = "select id from login_attempt_queue where last_checked > now() - interval $attempt_expiration_timeout*1000 MICROSECOND and username = '$this->username' order by id ASC limit 1"; // returns latest checked in record, this eliminates a large number of attempts done by hacker as only one of the many attempts is selected
